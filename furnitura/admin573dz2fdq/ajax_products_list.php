    ...
    foreach ($items as $item) {
        if (!$forceJson) {
            ...
            $results[] = array(
                ...
                'image' => str_replace('https://', Tools::getShopProtocol(), $context->link->getImageLink($item['link_rewrite'], $item['id_image'], 'home_default')),
            );
        }
    }
            ...
            if (!empty($combinations)) {
                foreach ($combinations as $k => $combination) {
                    ...
                    if (empty($results[$combination['id_product_attribute']]['image'])) {
                        $results[$combination['id_product_attribute']]['image'] = str_replace('https://', Tools::getShopProtocol(), $context->link->getImageLink($item['link_rewrite'], $combination['id_image'], 'home_default'));
                    }
                }
            } else {
                $results[] = array(
